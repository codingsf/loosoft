...
namespace Cn.Loosoft.Zhisou.SunPower.Domain
{
    ...
    public class Device
    {
        ...
        public float TotalEnergy
        {
            get
            {
                if (this.deviceTypeCode != DeviceData.INVERTER_CODE) return 0;
                if (this.deviceTypeCode == DeviceData.INVERTER_CODE && runData != null)
                {
                    float tmp = runData.getMonitorValue(MonitorType.MIC_INVERTER_TOTALENERGY);
                    if (float.IsNaN(tmp))
                    {
                        tmp = runData.getMonitorValue(MonitorType.MIC_INVERTER_ACENERGY);
                    }
        ...
        /// <param name="mtcode"></param>
        /// <returns></returns>
        public double getMonitorValue(int mtcode)
        {
            if (runData == null)
                return 0;
        ...
        public string getStatus()
        {
            ...
            {
                value = this.runData.getMonitorValueWithStatus(MonitorType.MIC_INVERTER_DEVICESTATUS);
            }
            else if (this.deviceTypeCode == DeviceData.HUILIUXIANG_CODE)
            {
                value = this.runData.getMonitorValueWithStatus(MonitorType.MIC_BUSBAR_STATUS);
            }
