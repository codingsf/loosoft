...
namespace DataAnalyze
{
    ...
    public class TcpDataProcesser
    {
        ...
        //modify by sungrow for 增加解析计数以便判断是否解析器是否在进行解析
        private static int ncountwork = 0;
        ...
        private void isshouhu()
        {
            ...
            //    LogUtil.writeline("计数" + ncountwork + ",小计数" + ncount);
            //    if (nwork == ncountwork)
            //    {
            //        ncount++;
            //    }
            ...
            {
                Thread.Sleep(10000);
                if (AnalyzeCount.total == 0) continue;//0不做处理
                LogUtil.writeline("上次解析总数" + lastTotal + ",现在" + AnalyzeCount.total + ",第" + nworknum + "次");
                if (AnalyzeCount.total == lastTotal)
                {
                    nworknum++;
                }
                else {
                    lastTotal = AnalyzeCount.total;
                    nworknum = 0;
                }

                if (nworknum>60)//大于10分钟则关闭程序
                {
                    //Process.GetCurrentProcess().Kill();
                    Environment.Exit(1);
                }
