...
namespace Cn.Loosoft.Zhisou.SunPower.Web.Controllers
{
    ...
    public class PlantChartController : BaseController
    {
        ...
        public ActionResult UserYearChart(int userId, string chartType)
        {
            ...
            if (user != null && user.displayPlants != null)
            {
                if (user.displayPlants.Count <= 2) chartType = ChartType.column;
                string unit = "kWh";// DeviceData.getMonitorItemByCode(monitorCode).Units[0];
                ChartData chartData = PlantChartService.GetInstance().YearChartBypList(user.displayPlants, chartType, unit);
                reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            }
            else
            {
        ...
        public ActionResult UserMonthDDChart(int userId, string startYYYYMMDD, string endYYYYMMDD, string chartType)
        {
            ...
            if (user != null && user.displayPlants != null)
            {
                ...
                string unit = "kWh";
                //取得用户年度发电量图表数据
                ChartData chartData = PlantChartService.GetInstance().MMDDChartBypList(user.displayPlants, startYYYYMMDD, endYYYYMMDD, chartType, unit);
                reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            }
            else
            {
        ...
        public ActionResult UserYearMMChart(int userId, string startYM, string endYM, string chartType)
        {
            ...
            if (user != null && user.displayPlants != null)
            {
                string unit = "kWh";
                //取得用户年度发电量图表数据
                ChartData chartData = PlantChartService.GetInstance().YearMMChartBypList(user.displayPlants, startYM, endYM, chartType, unit);
                reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
        ...
        public ActionResult PerformanceChart(int userId, string startYYYYMMDDHH, string endYYYYMMDDHH, string chartType, int intervalMins)
        {
            ...
            if (user != null && user.displayPlants != null)
            {
                ...
                float rate = 1.0F;
                Plant plant;
                for (int i = 0; i < user.displayPlants.Count; i++)
                {
                    plant = user.displayPlants[i];
                    rate = 1 / plant.chartPower;
                    if (user.displayPlants.Count <= 2)
        ...
        public ActionResult UserTotalKwpChart(int userId, string startYYYYMMDD, string endYYYYMMDD, string chartType)
        {
            ...
            if (user != null && user.displayPlants != null)
            {
                IList<int> tmpyears = collectorYearDataService.GetWorkYears(user.displayPlants);
        ...
        public ActionResult UserYearCompare(int userId, string chartType)
        {
            string reportCode = string.Empty;
            User user = UserService.GetInstance().Get(userId);
            if (user.displayPlants.Count > 0)
            {
                IList<int> workYears = CollectorYearDataService.GetInstance().GetWorkYears(user.displayPlants);
                ...
                ChartData chartData = CompareChartService.GetInstance().PlantYearCompare(user.displayPlants, workYears, Resources.SunResource.DEVICEMONITORITEM_11, mt, unit, chartType, 1.0F);
                reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            }
            else
            {
        ...
        public ActionResult UserMonthDDCompare(int userId, int mm, string chartType)
        {
            ...
            if (user.displayPlants.Count > 0)
            {
                ...
                ChartData chartData = CompareChartService.GetInstance().PlantMMCompare(user.displayPlants, yyyymms, Resources.SunResource.DEVICEMONITORITEM_11, mt, unit, chartType, 1.0F);
                reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            }
            else
            {
        ...
        /// <param name="startYYYYMMDDHH">开始年月日时</param>
        /// <param name="endYYYYMMDDHH">截止年月日时</param>
        /// <param name="chartType">图表类型</param>
        /// <param name="intervalMins">间隔时间</param>
        /// <returns></returns>
        [HttpPost]
        ...
        public ActionResult CompareDaykWpChartByPlant(int pId, string startYYYYMMDDHH, string endYYYYMMDDHH, string chartType, int intervalMins)
        {
            string reportCode = string.Empty;
            Plant plant = PlantService.GetInstance().GetPlantInfoById(pId);
            IList<PlantUnit> unitList = plant.allFactUnits;
        ...
        /// <param name="intervalMins"></param>
        /// <returns></returns>
        public ActionResult userFgpjDayChart(int uid, string YYYYMMDD, string chartType)
        {
            User user = UserService.GetInstance().Get(uid);
            ChartData chartData = PlantChartService.GetAppInstance().PlantDaySectionCountChart(user.allAssignedFactPlants, "天发电量和电费", YYYYMMDD, chartType, "kWh", MonitorType.PLANT_MONITORITEM_ENERGY_CODE, 5, 1);
            string reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            return Content(reportCode); 
        }
        ...
        /// <param name="intervalMins"></param>
        /// <returns></returns>
        public ActionResult PlantFgpjDayChart(int pid, string YYYYMMDD, string chartType)
        {
            Plant plant = PlantService.GetInstance().GetPlantInfoById(pid);
            ChartData chartData = PlantChartService.GetAppInstance().PlantDaySectionCountChart(plant.allFactPlants, "天发电量和电费", YYYYMMDD, chartType, "kWh", MonitorType.PLANT_MONITORITEM_ENERGY_CODE, 5, 1);
            string reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            return Content(reportCode); 
        }

        /// <summary>
        /// 用户分配的所有电站的实际电站峰谷平尖月分布图表
        /// </summary>
        /// <param name="uid"></param>
        ...
        /// <param name="intervalMins"></param>
        /// <returns></returns>
        public ActionResult userFgpjMonthChart(int uid, string YYYYMM, string chartType)
        {
            User user = UserService.GetInstance().Get(uid);
            ChartData chartData = PlantChartService.GetAppInstance().PlantMonthSectionCountChart(user.allAssignedFactPlants, "天发电量和电费", CalenderUtil.formatDate(DateTime.Now, "yyyyMMdd"), chartType, "kWh", MonitorType.PLANT_MONITORITEM_ENERGY_CODE, 5, 1);
            string reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            return Content(reportCode);
        }
        ...
        /// <param name="intervalMins"></param>
        /// <returns></returns>
        public ActionResult PlantFgpjMonthChart(int pid, string YYYYMM, string chartType)
        {
            Plant plant = PlantService.GetInstance().GetPlantInfoById(pid);
            ChartData chartData = PlantChartService.GetAppInstance().PlantMonthSectionCountChart(plant.allFactPlants, "天发电量和电费", YYYYMM, chartType, "kWh", MonitorType.PLANT_MONITORITEM_ENERGY_CODE, 5, 1);
            string reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            return Content(reportCode); 
        }

        /// <summary>
        /// 用户分配的所有电站的实际电站峰谷平尖年分布图表
        /// </summary>
        /// <param name="uid"></param>
        ...
        /// <param name="intervalMins"></param>
        /// <returns></returns>
        public ActionResult userFgpjYearChart(int uid, string year, string chartType)
        {
            User user = UserService.GetInstance().Get(uid);
            ChartData chartData = PlantChartService.GetAppInstance().PlantYearSectionCountChart(user.allAssignedFactPlants, "天发电量和电费", int.Parse(year), chartType, "kWh", MonitorType.PLANT_MONITORITEM_ENERGY_CODE, 5, 1);
            string reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            return Content(reportCode);
        }
        ...
        /// <param name="intervalMins"></param>
        /// <returns></returns>
        public ActionResult PlantFgpjYearChart(int pid, string year, string chartType)
        {
            Plant plant = PlantService.GetInstance().GetPlantInfoById(pid);
            ChartData chartData = PlantChartService.GetAppInstance().PlantYearSectionCountChart(plant.allFactPlants, "天发电量和电费", int.Parse(year), chartType, "kWh", MonitorType.PLANT_MONITORITEM_ENERGY_CODE, 5, 1);
            string reportCode = JsonUtil.convertToJson(chartData, typeof(ChartData));
            return Content(reportCode);
        }
    }
