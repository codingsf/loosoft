<%@ Page Title="" Language="C#" MasterPageFile="~/Pages/Shared/EmptyInside.Master"
    Inherits="System.Web.Mvc.ViewPage<Cn.Loosoft.Zhisou.SunPower.Domain.DefineReport>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div style="width: 98%; padding: 10px 10px 10px 10px; font-family: Verdana, Arial, Helvetica, sans-serif;
        ...
        <% 
            ...
            { 
        %>
        <table width="98%" border="1" bordercolor="#A8A8A8" cellpadding="0" cellspacing="0"
            style="border-collapse: collapse; line-height: 24px; text-align: center">
            <tr>
                <td colspan="10">
                    <strong>
                        <%=Model.ReportName%></strong>
                </td>
            </tr>
            <tr>
                <td colspan="10" align="right" style="padding-right: 10px;">
                    <%=Resources.SunResource.REPORT_VIEW_TIME %>：<%=ViewData["time"]%>
                </td>
            </tr>
            <tr>
                <td colspan="20" align="left" style="padding-left: 10px;">
                    <%=Resources.SunResource.REPORT_PLANT_COUNT_DATA%>
                </td>
            </tr>
            <!--no device data-->
            <% 
         ...
         if (tmpcode != 0)
         {
             ...
             keyvalue = plantHash[tmpcode].ToString();
                ...
                <td align="left" style="padding-left: 5px; border-right: 1.2,1,red">
                    <%=keyvalue%>
                </td>
                <% 
                                        ...
                                        <%
                                            if (ViewData["tId"].ToString().Equals("3"))
                                            {
                                                ...
                                                for (int p = 1; p <= 12; p++)
                                                {
                                                    ...
                                                    {
                                        %>
                                        <option value="<%=Convert.ToInt32(temp).ToString("00")%>" selected="selected">
                                            <%=Convert.ToInt32(temp).ToString("00")%></option>
                                        <%}
                                        else
                                        { %>
                                        <option value="<%=Convert.ToInt32(temp).ToString("00")%>">
                                            <%=Convert.ToInt32(temp).ToString("00")%></option>
                                        <%}
                                    }
                                }%>
    ...
    <div style="height: 15px; clear: both; width: 100%">
    </div>
</asp:Content>
