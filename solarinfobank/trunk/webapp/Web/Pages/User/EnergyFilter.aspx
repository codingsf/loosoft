...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <script type="text/javascript">
        ...
        function search() {
            plantId = $("#plant").val();
            window.location.href="/user/energyFilter?id=<%=Cn.Loosoft.Zhisou.SunPower.Service.UserUtil.getCurUser().id %>&pid="+$("#plant").val()+"&yyyyMMdd="+$("#endDate").attr("value");
        }
                    ...
                    <td width="15%">
                        <select name="plant" id="plant" class="txtbu01" style="width:120px">
                            ...
                            <% foreach (Plant plant in ViewData["plants"] as IList<Plant>)
                               {
                                   if (plant.id.ToString().Equals(Request["pid"]))
                                   {
                                   %>
                                <option value="<%=plant.id %>" selected=selected><%=plant.name%></option>
                                <%}
                                   else
                                   { %>
		              ...
		              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td><table width="100%" height="25" border="0" cellpadding="0" cellspacing="0" class="subline02">
                            <tr>
                              <td width="170" align="center"><strong><%=Resources.SunResource.USER_OVERVIEW_PLANTS%></strong> </td>
