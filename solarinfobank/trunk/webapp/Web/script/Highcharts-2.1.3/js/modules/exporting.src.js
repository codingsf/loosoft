...
//add by qhb for 多语言标题

function myexport() { // encapsulate
    // create shortcuts
    var HC = Highcharts,
	Chart = HC.Chart,
	...
	defaultOptions = HC.setOptions({
	    lang: {
	        downloadPNG: exportXlsTitle,
	        downloadJPEG: exportCsvTitle,
	        downloadPDF: exportPdfTitle,
	        downloadSVG: infoTitle,
	        printButtonTitle: 'Print the chart',
	        exportButton: exportButtonTitle,
	        largeButton: largeButtonTitle
	    }
	});
    ...
    //filename: 'chart',
        ...
        extend(Chart.prototype, {
            ...
            */
            exportChart: function(options, chartOptions) {
                ...
                //if (options.type == 'application/pdf') options.url = "http://export.highcharts.com/";
                // create the form
                ...
                var newname = options.filename;

                if (options.type == 'application/pdf') {
                    var newname = options.filename + options.chartname
                    svg = chart.getSVG(chartOptions);
                }
                // add the values
                each(['filename', 'type', 'width', 'svg', 'serieNo'], function(name) {
                    createElement('input', {
                        type: HIDDEN,
                        name: name,
                        value: {
                            filename: newname || 'chart',
                            type: options.type,
                            width: options.width,
                            svg: svg,
                            serieNo: options.serieNo
}[name]
                        }, null, form);
                    });

                    if (options.serieNo == null) return;
                    // submit
                    form.submit();
		...
		'Z'
            ...
            HC.Renderer.prototype.symbols.largeIcon = function(x, y, radius) {
                return [
		M, // the disk
		x - radius, y + radius,
		L,
