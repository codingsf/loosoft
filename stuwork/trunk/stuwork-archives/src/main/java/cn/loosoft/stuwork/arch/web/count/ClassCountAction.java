 ...
 */
@Namespace("/count")
@Results( { @Result(name = CrudActionSupport.RELOAD, location = "class-count.action", type = "redirect") })
public class ClassCountAction extends CrudActionSupport<Object>
{
    /**
    ...
    {
        this.classVOs = archivesManager.classCount();
        return SUCCESS;
    }
    ...
    public String printExcel() throws Exception
    {
        ...
        String titleStr = "学院;专业;班级;状态;人数;班级总人数";// 标题
        ...
        if (null != classVOs && classVOs.size() > 0)
        {
            for (CountVO countVO : classVOs)
            {

                ArrayList rowData = new ArrayList();
                rowData.add(countVO.getCollegeName());
                rowData.add(countVO.getMajorName());
                rowData.add(countVO.getClassName());
            ...
            try
            {
                ...
                // 不生成文件，直接生成文件输出流供下载
                downLoad.downLoad("档案统计-按班级统计.xls", excel, response);
    ...
    public List<CountVO> getClassVOs()
    {
        return classVOs;
    }
