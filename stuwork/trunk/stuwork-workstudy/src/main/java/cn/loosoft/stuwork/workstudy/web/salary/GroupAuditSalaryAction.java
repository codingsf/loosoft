...
@Namespace("/salary")
@Results( {
        @Result(name = CrudActionSupport.RELOAD, location = "group-audit-salary.action", type = "redirect"),
        @Result(name = "groupNoPass", location = "groupNoPass.jsp"),
        @Result(name = "nopasscheck-detail", location = "../job/nopasscheck-detail.jsp") })
public class GroupAuditSalaryAction extends CrudActionSupport<Salary>
{
    ...
    public String pass()
    {
        ...
        Struts2Utils.renderText("操作成功,小组审核工资通过");
        return null;
    }
    ...
    public String list() throws Exception
    {
        ...
        if (user != null)
        {
            if (user.getInstituteCode() != null)
            {
                filters.add(new PropertyFilter("EQS_collegeCode", user
                        .getInstituteCode()));
            }
        }
    ...
    {
        this.entity.setGroupStatus(Constant.SHWTG);
        salaryManager.save(entity);
    ...
    /**
     * 查看小组未过原因 Description of this Method
     * 
     * @since 2011-3-17
     * @author bing.hu
     * @return
