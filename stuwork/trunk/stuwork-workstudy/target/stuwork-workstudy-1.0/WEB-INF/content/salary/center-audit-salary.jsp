...
<div class="mid1003_r">
    ...
    <tr>
		<td bgcolor="#FFFFFF">
		<form id="searchForm" name="searchForm" action="${ctx}/salary/center-audit-salary.action" method="post">
		<table id="filter" width="100%" style="height: 35px" border="0" cellpadding="0" cellspacing="0" bgcolor="#F7E582" class="br07" >
		  ...
		  <tr>
			    <td  colspan="10">
			       		工作开始时间 从：<input name="startWorkStartTime" id="workStartTime"
				type="text" class="Wdate" class="ipt wid30 " onclick="WdatePicker()" />
		        到：<input name="endWorkStartTime" id="workStartTime"
			    type="text" class="Wdate" class="ipt wid30 " onclick="WdatePicker()" />
	     工作结束时间 从：<input name="startWorkStopTime" id="workStopTime"
				type="text" class="Wdate" class="ipt wid30 " onclick="WdatePicker()" />
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
	      <tr>
              <td height="30" colspan="12" bgcolor="#F7E582" class="br002"><strong>中心审核工资单</strong></td>
          </tr>
		  <tr>
		  ...
		  <tr>
		    ...
		    <td class="<s:if test='#status.even'>br05</s:if><s:else>br03</s:else>">${groupStatusDesc }&nbsp;</td> 
		        <td class="<s:if test='#status.even'>br05</s:if><s:else>br03</s:else>">${groupNoPassReason }&nbsp;</td>
		    ...
		    <td class="<s:if test='#status.even'>br05</s:if><s:else>br03</s:else>">
			  ...
			  <a href="${ctx}/salary/center-audit-salary!nopassrecheckDetail.action?id=${id}">查看</a></s:if></td>
			<td>
				<security:authorize ifNotGranted="ROLE_勤工助学_用工人员">
				<s:if test="centerStatus == 'shz'">
				<a href="javascript:void(0);" 
					onclick="pass('${id }','${ctx}/salary/center-audit-salary!pass.action');">通过</a>
				<a href="${ctx}/salary/center-audit-salary!unPass.action?id=${id}">不通过</a>
				</s:if>
				</security:authorize>
		    </td>
		  </tr>
